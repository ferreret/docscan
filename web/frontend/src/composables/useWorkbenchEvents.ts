...
export function useWorkbenchEvents(
  ...
  function applyResult(result: EventResult, payload: EventFireIn): void {
    if (
      ...
      Object.keys(result.fields_updated).length &&
      payload.page_id
    ) {
      options.onApplyPageFields?.(payload.page_id, result.fields_updated);
    }
    if (
      result.batch_fields_updated &&
      Object.keys(result.batch_fields_updated).length
    ) {
  ...
  function fireAsync(name: string, payload: EventFireIn = {}): void {
    ...
    lastFired.set(k, now);
    client
      .fireEvent(batchId, name, payload)
      .then((res) => applyResult(res, payload))
      .catch((e) => {
        const msg = e instanceof Error ? e.message : String(e);
        options.onLogs?.([
          { level: "error", message: `Evento ${name}: ${msg}` },
        ]);
      });
  }
