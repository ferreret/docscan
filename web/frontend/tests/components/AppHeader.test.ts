...
function makeRouter(initialPath: string) {
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/applications/:id/transfer', component: { template: '<div/>' } },
...
describe('AppHeader', () => {
  it('renderiza las 7 tabs con la activa correcta según la ruta', async () => {
    ...
    expect(tabs[5].text()).toBe('Transferencia')
    expect(tabs[6].text()).toBe('Eventos')
    expect(tabs[6].classes().join(' ')).toContain('text-primary')
  })
